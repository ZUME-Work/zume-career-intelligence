    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes shimmer { 0%,100% { box-shadow: 0 4px 20px rgba(79,70,229,0.25); } 50% { box-shadow: 0 8px 28px rgba(79,70,229,0.45); } }
        ...
        .btn-primary:hover { transform:translateY(-3px); box-shadow:0 10px 32px rgba(79,70,229,0.45); animation:none; }
        .btn-primary:active { transform:translateY(0); }
        .btn-secondary {
          display:inline-block; background:#fff; color:#333;
          padding:15px 36px; border-radius:12px; text-decoration:none;
          font-size:16px; font-weight:600; border:1.5px solid #E5E7EB;
          box-shadow:0 2px 8px rgba(0,0,0,0.06);
          transition:transform 0.18s, box-shadow 0.18s;
        }
        .btn-secondary:hover { transform:translateY(-3px); box-shadow:0 8px 24px rgba(0,0,0,0.1); }
        .btn-dark {
        ...
        .btn-dark:hover { transform:translateY(-3px); box-shadow:0 10px 28px rgba(0,0,0,0.3); }
        .btn-outline {
          display:inline-block; background:transparent; color:#fff;
          padding:15px 36px; border-radius:12px; text-decoration:none;
          font-size:16px; font-weight:600; border:1.5px solid #444;
          transition:transform 0.18s, border-color 0.18s;
        }
        .btn-outline:hover { transform:translateY(-3px); border-color:#888; }
        .skill-card {
          display:block; text-decoration:none; border-radius:16px; padding:24px 18px;
        ...
        .arch-card:hover { transform:translateY(-6px) !important; box-shadow:0 12px 28px rgba(0,0,0,0.1); animation:none; }
        .who-card {
      ...
      <main style={{ fontFamily:'system-ui,-apple-system,sans-serif', background:'#fff', minHeight:'100vh', color:'#111' }}>
        ...
        <nav style={{ borderBottom:'1px solid #eee', position:'sticky', top:0, background:'rgba(255,255,255,0.95)', backdropFilter:'blur(8px)', zIndex:100 }}>
          <div style={{ maxWidth:1100, margin:'0 auto', padding:'16px 40px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div style={{ fontWeight:800, fontSize:17, color:'#111' }}>
              ZUME <span style={{ color:'#4F46E5', fontWeight:500 }}>Datalab</span>
            </div>
            <Link href="/archetype" className="btn-primary" style={{ padding:'9px 22px', fontSize:14, animation:'none', boxShadow:'0 2px 10px rgba(79,70,229,0.25)' }}>
              ฉันเป็นสายไหน?
            </Link>
          </div>
        </nav>
        ...
        <section className="hero" style={{ maxWidth:1100, margin:'0 auto', padding:'80px 40px 72px' }}>
          <div style={{ maxWidth:600 }}>
            <h1 className="f1" style={{ fontSize:'clamp(2.2rem,4vw,3rem)', fontWeight:800, color:'#111', lineHeight:1.2, marginBottom:20, letterSpacing:'-0.5px' }}>
              คุณเป็นคนทำงาน<br />ด้าน Data สายไหน?
            </h1>
            <p className="f2" style={{ fontSize:18, color:'#666', lineHeight:1.8, marginBottom:36, maxWidth:440 }}>
              ค้นหาสไตล์การทำงานของคุณ แล้วทดสอบว่าแข็งแกร่งแค่ไหน
            </p>
            <div className="f3 btn-row" style={{ display:'flex', gap:14, flexWrap:'wrap' }}>
        ...
        <section className="section" style={{ background:'#F9F9F9', padding:'72px 40px' }}>
          <div style={{ maxWidth:1100, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:60, alignItems:'center' }}>
            <div>
              ...
              <h2 style={{ fontSize:'clamp(1.6rem,3vw,2rem)', fontWeight:800, color:'#111', marginBottom:14, lineHeight:1.3 }}>
                คนทำงาน Data มีหลายแบบ<br />แบบไหนคือคุณ?
              </h2>
              <p style={{ fontSize:17, color:'#666', lineHeight:1.8, marginBottom:28 }}>
        ...
        <section className="section" style={{ background:'#F9F9F9', padding:'72px 40px' }}>
          <div style={{ maxWidth:1100, margin:'0 auto' }}>
            <h2 style={{ fontSize:'clamp(1.6rem,3vw,2rem)', fontWeight:800, color:'#111', marginBottom:44, textAlign:'center' }}>เหมาะกับใครบ้าง?</h2>
        ...
        {/* CTA */}
        <section style={{ background:'#111', padding:'72px 40px', textAlign:'center' }}>
          <h2 style={{ fontSize:'clamp(1.6rem,3vw,2rem)', fontWeight:800, color:'#fff', marginBottom:12 }}>เริ่มได้เลย ใช้เวลาแค่ 10 นาที</h2>
          <p style={{ fontSize:17, color:'#666', marginBottom:36 }}>ฟรี · ไม่ต้องสมัครสมาชิก</p>
        ...
        <footer style={{ padding:'24px', textAlign:'center', background:'#111', borderTop:'1px solid #1a1a1a' }}>
